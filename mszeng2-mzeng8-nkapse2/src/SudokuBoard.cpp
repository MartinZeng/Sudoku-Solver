...
void SudokuBoard::populateVisitingOrder() {
    ...
    for(int degree = 8; degree >= 0; degree--) {
        ...
        for(int i = 0; i < (int)order_vect.size(); i++) {
            ...
            // std::cout << order_vect[i].row << ", " << order_vect[i].col << std::endl;
            ...
            //std::cout << "after checking saturation degree" << std::endl;
        }
        if(degree_exists) {
    ...
    for(int i = (int)(order_vect.size() - 1); i >= 0; i--) {
        if(order_vect[i].val == 0) {
            ...
            //std::cout << "---- (" << order_vect[i].row << ", " << order_vect[i].col << "), saturation degree: " << getGreedy(order_vect[i]) << std::endl;
        }
    }
}
...
void SudokuBoard::displayAdjacency() {
   ...
   for(int i = 0; i < 81; i++) {//the graph is symm so i cutoff half the graph for readability 
      for(int j = 0; j < 81; j++) { // j < count changed so I could see the whole adjaceny matrix
...
bool SudokuBoard::neighborsHaveColor(Cell cell, int color) {
    //std::cout << "checking if neighbors have color: (" <<cell.row << ", " << cell.col <<  "): " << color << std::endl;
    ...
    for(int i = 0; i < 81; i++) {
        if(adjacency_[current][i] == 1) {
            ...
            if(neighbor == color && !(neighbor_row == cell.row && neighor_col == cell.col)) {
                //std::cout << "RETURNING TRUE, "<< cells_[neighbor_row][neighor_col] << " found at: (" << neighbor_row << ", " << neighor_col << ")" << std::endl;
                //displayBoard();
                return true;
            }
        }
...
bool SudokuBoard::canColor(Cell cell, int color) {
    //std::cout << "examining: (" << cell.row << ", " << cell.col << ")" << std::endl;
...
void SudokuBoard::recursiveGraphColor(Cell cell) {
    //std::cout << "coloring: (" << cell.row << ", " << cell.col << ")" << std::endl;
...
//     // std::string test = "148 wouwor 148 jdofjdofj 148 pkadf==-=-";
//     // std::cout << test << std::endl;
//     // test = removeNonNumber(test);
//     // std::cout << test << std::endl;
//     SudokuBoard board(argv[1], 2);;
